sap.ui.define([
	...
	return Controller.extend("tool_roro_m30.controller.FK10N", {
		onInit: function () {
			...
			this._oVendor = this.getView().byId("vendor");
			...
			var variantModel;
			oPersonalizationService.getContainer("sap.ushell.variant.FK10N_roro_m30", oScope, oComponent)
				.fail(function () {
		...
		onPressNext: function (oEvent) {
			...
			var oVendor = this._oVendor.getTokens()[0].getKey();
			...
			this._getCompanyModel(oClient, oCCode, oVendor, oYear).then(function (oDataColumn) {
			...
			this._modelCalculation(oClient, oCCode, oVendor, oYear).then(function (oModel) {
				var oTableModel = oView.byId("tableId");
				oView.setModel(oModel, "data");
				//	oTableModel.setModel(oModel);
			});
			this._modelCalculationSL(oClient, oCCode, oVendor, oYear).then(function (oModel) {
		...
		_getCompanyModel: function (oClient, oCCode, oVendor, oYear) {
		...
		_getEntityName: function (oDataColumn) {
			...
			var aCustomerFilter = new sap.ui.model.Filter({
				path: "LIFNR",
				operator: sap.ui.model.FilterOperator.EQ,
				value1: oDataColumn.company[0].columnVendor
			});
				...
				oF10Model.read("/lfa1", {
					...
					success: function (oResult) {
						for (var i = 0; i < oResult.results.length; i++) {
							...
							oDataColumn.company[0].columnVendor = (oDataColumn.company[0].columnVendor+ " " + property.NAME1 + " " + property.NAME2);
		...
		_modelCalculation: function (oClient, oCCode, oVendor, oYear) {
			var that = this;
			var oComponent = this.getOwnerComponent(); //Returns the Component
			var oF10Model = oComponent.getModel("F10Set");
			...
			return new Promise(function (resolve, reject) {
				...
				oF10Model.read(
					"/fk10nParameters(IP_MANDT='" + oClient + "',IP_LIFNR='" + oVendor + "',IP_BUKRS='" + oCCode + "',IP_GJAHR='" + oYear +
					"')/Results", {
						success: function (oResult) {
		...
		_modelCalculationSL: function (oClient, oCCode, oVendor, oYear) {
			var that = this;
			var oComponent = this.getOwnerComponent(); //Returns the Component
			var oF10Model = oComponent.getModel("F10Set");
			//			var bcf = this.getOwnerComponent().getView().getModel("i18n").getResourceBundle().getText("bcf");
			...
			return new Promise(function (resolve, reject) {
				...
				oF10Model.read(
					"/fd10nslParameters(IP_MANDT='" + oClient + "',IP_KUNNR='" + oVendor + "',IP_BUKRS='" + oCCode + "',IP_GJAHR='" + oYear +
					"')/Results", {
						success: function (oResult) {
							var oLocale = sap.ui.getCore().getConfiguration().getLanguage();
		...
		handleValueHelpCompany: function () {
			if (!this._oValueHelpDialog) {
				Fragment.load({
					name: "tool_roro_m30.view.ValueHelpCompany",
					controller: this
		...
		handleValueHelpCustomer: function () {
			if (!this._oValueHelpDialogCustomer) {
				Fragment.load({
					name: "tool_roro_m30.view.ValueHelpCustomer",
					controller: this
		...
		showCompanyCode: function () {
			...
			this._oValueHelpDialogCompanyCode = sap.ui.xmlfragment("tool_roro_m30.view.fragment.CompanyCodeFragment", this);
		...
		showVendor: function () {
			...
			e.setData({
				cols: [{
					...
					label: self.getView().getModel("i18n").getResourceBundle().getText("vendor"),
					template: "LIFNR"
				}, {
					label: self.getView().getModel("i18n").getResourceBundle().getText("companycode"),
					template: "BUKRS"
				}, {
					label: self.getView().getModel("i18n").getResourceBundle().getText("vendorname"),
					template: "NAME1"
				}]
			});
			var t = e.getData().cols;
			this._oBasicSearchFieldVendor = new SearchField({
				showSearchButton: false
			});
			this._oValueHelpDialogVendor = sap.ui.xmlfragment("tool_roro_m30.view.fragment.VendorFragment", this);
			this.getView().addDependent(this._oValueHelpDialogVendor);
			this._oValueHelpDialogVendor.setRangeKeyFields([{
				label: self.getView().getModel("i18n").getResourceBundle().getText("vendor"),
				key: "LIFNR",
				type: "string",
				typeInstance: new String({}, {
			...
			this._oValueHelpDialogVendor.getTableAsync().then(function (t) {
				var aFilters = [];
				var o = this.getView().byId("client")._getSelectedItemText();
				if (o == "") {
					o = null;
				...
				var a;
				this._oValueHelpDialogVendor.getFilterBar().setBasicSearch(this._oBasicSearchFieldVendor);
				BusyIndicator.show(0);
				this.getView().getModel().read("/vendor", {
					filters: aFilters,
					success: function (e, o) {
						a = new sap.ui.model.json.JSONModel(e.results);
						t.setModel(a);
				...
				t.setModel(e, "columns");
				this._oValueHelpDialogVendor.update();
				BusyIndicator.hide();
			}.bind(this));
			this._oValueHelpDialogVendor.setTokens(this._oVendor.getTokens());
			this._oValueHelpDialogVendor.open();
		},
		onValueHelpOkPressVendor: function (e) {
			var t = e.getParameter("tokens");
			this._oVendor.setTokens(t);
			this._oValueHelpDialogVendor.close();
		},
		onValueHelpCancelPressVendor: function () {
			this._oValueHelpDialogVendor.close();
		},
		onValueHelpAfterCloseVendor: function () {
			this._oValueHelpDialogVendor.destroy();
		},
		onFilterBarSearchVendor: function (e) {
			var t = this._oBasicSearchFieldVendor.getValue(),
				o = e.getParameter("selectionSet");
			var a = o.reduce(function (e, t) {
				if (t.getValue()) {
					e.push(new Filter({
			...
			this._filterTableVendor(new Filter({
				filters: a,
				and: true
			}));
		},
		_filterTableVendor: function (e) {
			var t = this._oValueHelpDialogVendor;
			t.getTableAsync().then(function (o) {
				if (o.bindRows) {
					o.getBinding("rows").filter(e);
				}
		...
		resetFields: function () {
			this.getView().byId("vendor").destroyTokens();
		...
		onSaveVariant: function (oEvent) {
			var variant = {
				...
				"vendor": {
					"key": this._oVendor.getTokens()[0].getKey(),
					"text": this._oVendor.getTokens()[0].getText()
				},
		...
		onChange: function (oEvent) {
			...
			this._oVendor.setTokens(
				[new sap.m.Token({
					key: changedVariant.vendor.key,
					text: changedVariant.vendor.text
				})]
			);
